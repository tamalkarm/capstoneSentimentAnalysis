...
class SentimentRecomModel:

    ROOT_PATH = "pickle/"
    MODEL_NAME = "sentimentBased-xg-boost-model.pkl"
    VECTORIZER = "tfidf-vectorizer.pkl"
    RECOMMENDER = "user_based_rating.pkl"
    CLEANED_DATA = "cln-data.pkl"

    def __init__(self):
        self.model = pickle.load(open(
            SentimentRecomModel.ROOT_PATH + SentimentRecomModel.MODEL_NAME, 'rb'))
        self.vectorizer = pd.read_pickle(
            SentimentRecomModel.ROOT_PATH + SentimentRecomModel.VECTORIZER)
        self.usr_final_rating = pickle.load(open(
            SentimentRecomModel.ROOT_PATH + SentimentRecomModel.RECOMMENDER, 'rb'))
        self.data = pd.read_csv("data/sample30.csv")
        self.cleaned_data = pickle.load(open(
            SentimentRecomModel.ROOT_PATH + SentimentRecomModel.CLEANED_DATA, 'rb'))
    ...
    def getRecommendationByUser(self, user):
        recommedations = []
        return list(self.usr_final_rating.loc[user].sort_values(ascending=False)[0:20].index)
    ...
    def getSentimentRecommendations(self, user):
        if (user in self.usr_final_rating.index):
            # get the product recommedation using the trained ML model
            recommendations = list(
                self.usr_final_rating.loc[user].sort_values(ascending=False)[0:20].index)
